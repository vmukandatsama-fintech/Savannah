...
def post_return_stock(collection_number, returned_by, reason, line_items):
    ...
    print("[DEBUG] post_return_stock arguments:")
    print("  CollectionNumber:", collection_number)
    print("  ReturnedBy:", returned_by)
    print("  ReturnJson:", payload)
    print("  Reason:", reason)
    with connection.cursor() as cursor:
        cursor.execute(
            "EXEC dbo.sp_ReturnIssuedStock @CollectionNumber=?, @ReturnedBy=?, @ReturnJson=?, @Reason=?",
...
def post_return_stock(collection_number, returned_by, reason, line_items):
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            "EXEC dbo.sp_ReturnIssuedStock @CollectionNumber=%s, @ReturnedBy=%s, @Reason=%s, @ItemsJson=%s",
            [collection_number, returned_by, reason, payload],
        )
        if cursor.description:
            rows = _fetch_all_as_dict(cursor)
            return rows[0] if rows else None
